...
## Test  '/courses/{course_id}/{chapter_id}'
...
def test_get_chapter_info_not_exists():
    response = client.get("/courses/1/5")
    assert response.status_code == 404
    assert response.json() == {'detail': 'Chapter not found'}
    
## Test '/courses/{course_id}/{chapter_id}?rating=
...
def test_rate_chapter_not_exists():
    response = client.post("/courses/1/500/rate", json={"rating": 1})
    assert response.status_code == 404
